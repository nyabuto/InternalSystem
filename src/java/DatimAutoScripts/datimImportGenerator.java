 ...
 */
public class datimImportGenerator extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //this is the mecahnism ID for Tujenge Jamii. Change this any time you have a new mechanism in place
        String attributeOptionCombo="h8CucOi0cWq";
        String un="";
        String pw="";
        String ds="";
        String prd="";
...
//               {
//              
             enterAnyData(conn,prdhm,scts_arr[b],forms_arr[ay],attributeOptionCombo);
//               }
...
//} //end of checking if site name is valid
//} //end of sites loop  
...
//wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'data-entry-form')))
    ...
    public String getServletInfo() {
      ...
      public  void enterAnyData(dbConn conn,HashMap<String,String> basics, String section, String Storedprocedure,String MechanismId) throws InterruptedException{
        ...
        try {
            ...
            //on the hashmap, add a metadata containing the columns and create a coma separated string that should be used to split the variables into an array
            //get th ename of the stored procedure
            
            String datim_import_sp="";
            ...
            while(conn.rs1.next())
            {
                datim_import_sp=conn.rs1.getString("datim_import_sp");
            ...
            System.out.println("Query Being Execured::"+datim_import_sp);
            String startd=basics.get("sd");
            String quarter=basics.get("prd");
            ...
            if((reportingperiod.equals("QA"))|| (reportingperiod.equals("SA") && (startd.contains("-01-01") || startd.contains("-07-01") )) || (reportingperiod.equals("AN") && (startd.contains("-07-01") )) ){
                ...
                try {
                    String qry="call internal_system."+datim_import_sp+"(\""+basics.get("sd")+"\",\""+basics.get("ed")+"\",\""+basics.get("hf")+"\");";
                    
                    System.out.println("query:"+qry);
                    
                    HashMap<String, String> alldata = new HashMap< >();
                                        
                    alldata=convertDatimResultSetToMapWithPartner(MechanismId,getAnyDataFromDb(conn,qry));
...
// Iterate through keys and values
        for (Map.Entry<String, String> entry : alldata.entrySet()) {
            ...
            String elementvals[]=keyvals[1].split("-"); //XboLlTkc4Av-Z1hcjaGpO6K-val
            //System.out.println("element vals being split:"+keyvals[1]);
            if(elementvals.length>1 && new Double(value)>0){
                            String dataelement=elementvals[0];
                            String period=quarter;
                            String orgunit=keyvals[0];	
                            String categoryoptioncombo=elementvals[1];	
                            String attributeoptioncombo=MechanismId;
                            String finalvalue=value;
            String id=period+"_"+dataelement+"_"+orgunit+"_"+categoryoptioncombo;  //#prd_dataelement_orgunit_categoryoptioncombo
              
            String replacedata=" replace into internal_system.datim_data_import (id,dataelement,period,orgunit,categoryoptioncombo,attributeoptioncombo ,value) "
                    + "values ('"+id+"','"+dataelement+"','"+period+"','"+orgunit+"','"+categoryoptioncombo+"','"+attributeoptioncombo+"','"+finalvalue+"')";
                System.out.println("qry:"+replacedata);
            conn.st3.executeUpdate(replacedata);
            // Use them dynamically in your code
...
//                                    act.moveToElement(driver.findElement(By.id(clmnar[cl]))).click().perform();
                                  ...
                                  //  if(!clmnar[cl].equals("Facility") && !clmnar[cl].equals("datimname") ){
                                       //now here we are looping through each column 
                                        
                                     //   String val="";
                                      //   if(alldata.get(dfacil+"_"+clmnar[cl])!=null){ 
                                         //here, create a whole row that contains
                       
                            
                           // String dataelement="";
                            
                            
                                           //  val=alldata.get(dfacil+"_"+clmnar[cl]); 
...
private HashMap<String,String> convertDatimResultSetToMapWithPartner(String MechanismId,ResultSet rs) throws SQLException{
...
public static String createCsvAndReturnLastRow(String filePath, List<String> headers, List<List<String>> rows) throws IOException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(csvFile))) {
            ...
            for (List<String> row : rows) {
                ...
                writer.write(line);
                writer.newLine();
                lastRow = line; // Keep track of the last one
            }
