 ...
 */
public class allformsreportstracker extends HttpServlet {
HttpSession session;
...
String currentCounty,prevCounty;
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       //Maureen to add more conditions in 711 or
        ...
        if(form.equalsIgnoreCase("KMMP")){  orquery=kmmpor;}
        else if(form.equalsIgnoreCase("VMMC")){  orquery=vmmcor;}
        else if(form.equalsIgnoreCase("Gender")){  orquery=genderor;}
        else if(form.equalsIgnoreCase("Nutrition")){  orquery=nutritionor;}
        else if(form.equalsIgnoreCase("MOH 711A")){  orquery=form711or;}
        else if(form.equalsIgnoreCase("TB")){  orquery=tbor;}
        else{ orquery=" subpartnera.PMTCT=1 OR subpartnera.ART=1 OR subpartnera.PEP=1 "; } //731
...
//        year=2015;
       prevYear=year-1; 
        ...
        allMonths.clear();
        allReports.clear();
        duration="WHERE ("+tablename+".yearmonth BETWEEN "+prevYear+""+10+" AND "+year+"09) AND ( "+orquery+" )";
...
styleHeader.setWrapText(true);
   ...
   HSSFCell  S1cell=rw1S1.createCell(0);
    ...
    counterHeader=5;
    String getMaxandMin="SELECT DISTINCT(month.name),month.id FROM "+tablename+" JOIN month ON "+tablename+".Mois=month.id JOIN subpartnera ON "+tablename+".SubPartnerID=subpartnera.SubPartnerID "+duration+" "
    + " ORDER BY  "+tablename+".yearmonth";
...
currentCounty=prevCounty="";
   ...
   if(allMonths.size()>0)
   {
     ...
     + "subpartnera.CentreSanteId,COUNT("+tablename+".SubPartnerID),month.name,subpartnera.SubPartnerID "
    + "FROM subpartnera "
    + "LEFT JOIN "+tablename+" ON "+tablename+".SubPartnerID=subpartnera.SubPartnerID "
    + "LEFT JOIN district ON subpartnera.DistrictID=district.DistrictID "
    + "LEFT JOIN county ON county.CountyID=district.CountyID "
    + "LEFT JOIN month ON "+tablename+".Mois=month.id "
    + " "+duration+" "
    + " GROUP BY subpartnera.SubPartnerNom,"+tablename+".Annee,"+tablename+".Mois "
    + "ORDER BY county.County,district.DistrictNom,subpartnera.SubPartnerNom,"+tablename+".Mois";
...
response.setHeader("Content-Disposition", "attachment; filename="+tablename+"_ReportsTracker_YEAR("+year+")_Generated_on_"+createdOn.trim()+".xls");
