 ...
 * @author EKaunda
 */
public class ValidateExcelJMW extends HttpServlet {
   ...
   dbConn conn = new dbConn();
    ...
    public boolean data_exist(String facil_id, String yearmonth) throws SQLException{
        ...
        int validations_ni_ngapi=0;
         String countRules="SELECT count(validation) FROM jmw_validation WHERE active=1 AND source='jmw'   ";
...
//           System.out.println("called to validate");
        ...
        String getRules="SELECT section_name,codes,validation,iscritical,message,fine_age,raw_query FROM jmw_validation WHERE active=1 AND source='jmw' order by section_name  ";
       ...
       // System.out.println("final query :"+final_query);
        ...
        conn.rs1 = conn.st1.executeQuery(final_query);
          ...
          if(conn.rs1.getString("fine_age").equals("1")){
            age_groups = "<1 F,<1 M,1-4 F,1-4 M,5-9 F,5-9 M,10-14 F,10-14 M,15-19 F,15-19 M";
           }
        ...
        switch (fine_age) {
            case 1:
                query+= "(SUM(IFNULL(CASE WHEN indicator_id IN(lhs) THEN f_1 END,0)) sign SUM(IFNULL(CASE WHEN  indicator_id IN(rhs) THEN f_1 end,0))) +\n" +
                        ...
                        "(SUM(IFNULL(CASE WHEN indicator_id IN(lhs) THEN m_19 END,0)) sign SUM(IFNULL(CASE WHEN  indicator_id IN(rhs) THEN m_19 end,0))) as occurences ";
