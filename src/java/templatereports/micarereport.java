 ...
 * @author EKaunda
 */
public class micarereport extends HttpServlet {
...
String allpath = getServletContext().getRealPath("/micare.xlsx");
...
String sourcepath = getServletContext().getRealPath("/micare.xlsx");
...
((XSSFSheet)sheet).getCTWorksheet().getDimension().setRef("A1:N" + (shet.getLastRowNum() + 1));

CTTable ctTable = ((XSSFSheet)sheet).getTables().get(0).getCTTable();

ctTable.setRef("A1:N" + (sheet.getLastRowNum() + 1)); // adjust reference as needed
