 ...
 * @author EKaunda
 */
public class liftupmicareanalysis extends HttpServlet {
...
String reportdetails[]={"LiFT Report Output@internal_system.sp_kp_liftup_raw('sd');@A1:AF","rawdata@internal_system.sp_kp_liftup_micare_rawfile('sd','ed');@A1:U"};


String allpath = getServletContext().getRealPath("/liftup_micare_analysis.xlsx");
...
String filepath="MICLIFT_"+dat2+".xlsx";
...
String sourcepath = getServletContext().getRealPath("/liftup_micare_analysis.xlsx");
...
XSSFWorkbook wb = wb1;
        ...
        for(int ax=0;ax<reportdetails.length;ax++)
        {
        
        String curparams[]=reportdetails[ax].split("@");
        //LiFT Report Output@internal_system.sp_kp_liftup_raw('sd');@A1:AF
          
        
        
        XSSFSheet shet = wb.getSheet(curparams[0]);
 ...
 eym=enddate.replace("-","").substring(0, 6);
        ...
        **/
        ...
        String qry = "call "+curparams[1].replace("sd", sym).replace("ed", eym);
       // String qry = "call sp_kp_micare_rawdata_dynamic('"+sym+"','"+eym+"');";
...
//                    cell0.setCellStyle(stylex);
   ...
   if(1==1){
     XSSFSheet sheet= wb.getSheet(curparams[0]);
        // tell your xssfsheet where its content begins and where it ends
((XSSFSheet)sheet).getCTWorksheet().getDimension().setRef(curparams[2]+"" + (shet.getLastRowNum() + 1));

CTTable ctTable = ((XSSFSheet)sheet).getTables().get(0).getCTTable();

ctTable.setRef(curparams[2]+"" + (sheet.getLastRowNum() + 1)); // adjust reference as needed
        ...
        System.out.println("" + "Micare_liftup_reports_Gen_" + createdOn.trim() + ".xlsx");
        ...
        response.setHeader("Content-Disposition", "attachment; filename=" + "Micare_liftup_Report_btwn_"+sym+"_At_"+eym+"_gen_" + createdOn.trim() + ".xlsx");
