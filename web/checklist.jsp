...
<script type="text/javascript" src="js/mnecl/sum_values.js"></script>
<script type="text/javascript" src="js/mnecl/validation.js"></script>
</head>
<!-- END HEAD -->
<!-- BEGIN BODY -->
...
<!--                                   <font color="#4b8df8" size="3px"><b><%if(session.getAttribute("year")!=null){out.println(session.getAttribute("year").toString()+" | ");}%></b></font>
  ...
  <!----------------------------------------------------------------------------services modal-------------------------------------------------------------->
                  ...
                  <table>  
                      <!--<tr><th>Element</th><th>Value</th><th>Element</th><th>Value</th></tr>-->
                   
                
                   
                                 <tr>
                                     <td style="width:10%;">   <label style="color:black;font-size: 15px;"><font color="red"><b>*</b></font>Date of Verification</label></td>
                                     <td style="width:30%;">
                                    <div class="controls">
                                    <input type="text" onchange="saveheadings('verificationdate','verificationdate');"  name ="verificationdate" id="verificationdate"  class="form-control tarehe1" readonly placeholder="e.g yyyy-mm-dd">
                                    </div>
                                     </td>
                                     <td style="width:15%;" > <label style="color:black;font-size: 15px;"><font color="red"><b>*</b></font>Name of Staff(s) involved</label></td>
                                     <td style="width:45%;" >
                                    
                                    <div class="controls">
                                     <textarea onchange="saveheadings('staff','staff');" type="text"  name ="staff" id="staff"  class="form-control"  placeholder="e.g john doe,John Alex "></textarea>>
                                    </div></td>
                                 
                                 </tr>
                                 <tr><td colspan='4'><label id="muwekadata" style="color:green;font-size: 12px;text-align: left;"></label></td></tr>
...
<!--                  <tr><td colspan='2'><div class="control-group">
                        ...
                        <div class="accordion" id="form1a_accordion">
                            <div style="color:#000000; font-size: 30px; font-weight: 500; text-align: center; margin-top: 10%;"> Please wait, we are loading DQA Checklist form ... <img src="images/utube.gif"></div>
                       </div>   
                        <!-- END FORM-->           
                     </div>
                  </div>
...
//                        });
                        ...
                        function saveheadings(level1,dcolumnname)
                        {
       ...
       if(val!=='') {
                        ...
                        $.ajax({
            url:'savevisitdetails?value='+val+"&col="+dcolumnname,
            type:'post',
            dataType:'html',
            success:function (data){
    ...
    function loaddates(){
          ...
          $.ajax({
            url:'loadvisit_Details',
            type:'post',
            dataType:'json',
            success:function (data){
                
                $("#verificationdate").val(data.verificationdate);
                $("#staff").val(data.staff);
                var tareheyakuanza=data.mindate;
                
                if(data.dataposter!==''){
                $("#muwekadata").html("Entry done by: "+data.dataposter);
            }
            else { $("#muwekadata").html("New Form "); }
//                console.log("Minimum date "+tareheyakuanza);
                
                 $('.tarehe1').datepicker({todayHighlight: true, clearBtn: true, autoHide: true, dateFormat: "yy-mm-dd",minDate: new Date(tareheyakuanza),maxDate: "now()",changeMonth: true,changeYear: true});
       ...
       function concordance(reportedval,verifiedval,destination){
           
         var rptd=$("#"+reportedval).val();
         var vrfd=$("#"+verifiedval).val();
           ...
           vrfd=parseInt(vrfd);
             ...
             var difference =  Math.abs(rptd - vrfd); 
            ...
            var conc= Math.round(((difference/vrfd)*100));
            if(isNaN(conc)){
         ...
         console.log("matokeo "+reportedval+" "+rptd+" "+verifiedval+" "+vrfd);  
       ...
       function isDateEntered(){
           ...
           var vl=$("#verificationdate").val();
//                      alert(vl);
           if(vl==='' || vl==='yyyy-mm-dd'){
               $("#verificationdate").focus();
               $("#verificationdate").css({"border-color": "red"});
        alert("Please enter Verification Date and Name of Staff(s) participating");   
        }
       }
