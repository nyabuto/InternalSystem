...
<div id="accordion">
  ...
  <h3 style="text-align: center;" id="participantsaccordion"><span class='badge badge-warning'>2</span> NASCOP EID/VL Data Cleaning Module</h3>
  <div>     
                                    ...
                                    <div class="controls ">
                                       <table style='margin-left: 0%;background-color: gainsboro;width:70%;'> 
                                        ...
                                        <td><select onchange='displayIncorrectCcc();'  name ="facility" id="facility"  class="form-control" >
                                            <option value=''></option>
                                        </select></td><td><a  class='btn btn-info' style='margin-left: 100px;'  data-toggle='modal' href='#rawdata'><i class='icon-plus'></i>Download Excel Reports</a></td>
                                           </tr>
                                       </table>
                                    </div>
                                </div> 
                                   ...
                                   <td>
                                    ...
                                    <div class="controls">
                                        <select name='reporttype' id='reporttype'>                                           
                                            <option value='sp_vl_get_noncleanvl_notupdated'>Not Updated Records</option>
                                             <option value='sp_vl_get_noncleanvl_updated'>Updated Records Only</option>
                                            <option value='sp_vl_get_noncleanvl_all'>All Records</option>
  ...
  var sexelement=participantid.replace("tablerow","sex");
   ...
   // });
            ...
            function getSitesWithNonStandardCccNos() {
                ...
                $.ajax({
                    url: 'getSitesWithNonStandardCccNos',
                    type: 'post',
                    dataType: 'json',
                    success: function (data) {
                        ...
                        for(var a=0;a<dt.length;a++){
                         ...
                         sites+="<option value='"+dt[a].facility_code+"'>"+dt[a].facility+"</option>";
  ...
  $( function() {
    ...
    function displayIncorrectCcc() {
              ...
              $("#hcservices").html('<img src="images/ajax_loader.gif"/>');
                ...
                $.ajax({
                    url: 'load_unclean_data?mflcode_sync='+facility,
                    type: 'post',
                    dataType: 'json',
                    success: function (data) {
console.log(data);
var dt=data[0].vl_for_cleaning;
//table should have ccc Number, 

 var tbl="<table id='participants2table' class= 'table  table-bordered table-responsive' style='width:100%;'><thead><tr><th>Patient Details</th><th><font color='green'>Correct CCC Number</font></th><th><font color='green'>Correct PMTCT Type</font></th></tr></thead><tbody>";
        
        
        for(var i=0;i<dt.length;i++)
        {
            var cccnos=dt[i].patient_ccc_no;
            var pmtctoptions=pmtctoptionvalues(dt[i].new_pmtct,dt[i].gender);
            tbl+="<tr id='"+dt[i].system_id+"'>\n\
<td><b>Old CCC No:</b><font color='red'>"+dt[i].patient_ccc_no+"</font><br/><b>Sex:</b> "+dt[i].gender+"<br/><b>DOB:</b> "+dt[i].dob+"<br/><b>Sample date:</b>"+dt[i].date_collected+"<br/></b><b>PMTCT:</b>"+dt[i].pmtct+"<br/> CCC # length: </b>"+cccnos.length+"</td>\n\
<td><input value='"+dt[i].new_cccno+"' onkeyup='isCCCComplete(\""+i+"\");'  style='width:95%;' id='newccc"+i+"' name='newccc"+i+"' onchange='updateValue(\""+dt[i].system_id+"\",\"newccc"+i+"\",\"ccc\");' type='text' maxlength='10' minlength='10' class=''><label id='noofdigits"+i+"'></label></td><td><select style='width:100%;' id='newpmtct"+i+"' name='newpmtct"+i+"' onchange='updateValue(\""+dt[i].system_id+"\",\"newpmtct"+i+"\",\"pmtct\");'> "+pmtctoptions+"</select></td></tr>";
...
//    } );
 ...
 function pmtctoptionvalues(curoption,sex)
        
    {   
    
    var sel="<option value=''>option</option>";
    var opt=["Pregnant","Breast Feeding","No Data","None of above"];
    var op1=["No Data"];
    ...
    sel+="<option "+selec+" value='"+op[nm]+"'>"+op[nm]+"</option>";
    }
  ...
  function updateValue(systemid,elem,elem_request_name)
  {
       ...
       if(elem.includes("newccc"))
       {   
            console.log("CCC number has been updated");
        if(isCCCNumberComplete(elementvalue)) 
        {
         //save          
           blackbordercl(elem); 
          proceed=true;  
      ...
      // alert(da);
            ...
            if(proceed){
                $.ajax({
                    url: 'updateNewRecords?sid='+sid+"&"+elem_request_name+"="+elementvalue,
                    type: 'post',
                    dataType: 'html',
                    success: function (data) {                      
...
function getreport(){
  ...
  function isCCCNumberComplete(ccc){
      ...
      {
       cccnumberokay=false; 
       msg="<font color='red'>Error: CCC number <b>"+ccc+"</b> is not 10 digits long!</font>";
       console.log(msg);
       $("#ujumbempya").html(msg);
      }
      
      else if(ccc.includes("-") || ccc.includes("/") || ccc.includes(".") || ccc.includes(" ") )
      {          
          cccnumberokay=false;
          msg="<font color='red'>Error: CCC number <b>"+ccc+"</b> contains special characters</font>";
       console.log(msg);
      $("#ujumbempya").html(msg);
      }
      ...
      {          
          cccnumberokay=false;
          msg="<font color='red'>Error: CCC number <b>"+ccc+"</b> contains non numeric characters</font.";
       console.log(msg);
       $("#ujumbempya").html(msg);
      }
      else
      ...
      return cccnumberokay;
  }
  
  
  
    function isCCCComplete(rowpos)
     {
         
         var noofdigits="noofdigits"+rowpos;
         var cccelement="newccc"+rowpos;
         
         
    var retval=false;
var ccc_number=$("#"+cccelement).val();
...
$("#"+cccelement).css('border-color','red'); 
...
$("#"+cccelement).css('border-color','#33b775'); 
