...
<html>
    ...
    <head>
        <meta charset="utf-8" />
        <title>CalHIV Data Cleaning</title>
...
<div id="accordion">
  ...
  <h3 style="text-align: center;" id="participantsaccordion"><span class='badge badge-warning'>2</span> CALHIVs Regimens Data Cleaning Module</h3>
  <div>     
  ...
  $( function() {
    ...
    function displayIncorrectCcc() {
              ...
              $("#hcservices").html('<img src="images/ajax_loader.gif"/>');
                ...
                $.ajax({
                    url: 'load_unclean_calhiv?mflcode_sync='+facility,
                    type: 'post',
                    dataType: 'json',
                    success: function (data) {
console.log(data);
var dt=data[0].calhiv_cleaning;
//table should have ccc Number, 

 var tbl="<table id='participants2table' class= 'table  table-bordered table-responsive' style='width:100%;'><thead><tr><th style='width:30%;'>CALHIV Details</th><th><font color='green'>Correct Regimen</font></th><th style='width:20%;'><font color='green'>Correct Regimen Line</font></th><th style='width:20%;'><font color='green'>Correct Last Visit Weight</font></th></tr></thead><tbody>";
        
        
        for(var i=0;i<dt.length;i++)
        {
            var weights=dt[i].Last_Visit_Weight;
            var regimenlineoption=regimenlineoptionvalues(dt[i].Current_Regimen_Line);
            var regimenoption=regimenoptionvalues(dt[i].Current_Regimen);
            tbl+="<tr id='"+dt[i].Patient_Id+"'>\n\
<td><b>CCC No:</b><font color='green'>"+dt[i].ccc_number+"</font><br/><b>Required Action:</b><font color='red'>"+dt[i].Required_action+"</font><br/><b>Last Visit date:</b>"+dt[i].Last_Clinic_Visit_Date+"<br/></b><b>Next TCA Date:</b>"+dt[i].Next_Appointment_Date+"<br/> Error Status: </b>"+dt[i].status+"<br/></td><td><select style='width:100%;' id='newreg"+i+"' name='newreg"+i+"' onchange='updateValue(\""+dt[i].Patient_Id+"\",\"newreg"+i+"\",\"regimen\");'> "+regimenoption+"</select></td><td><select style='width:100%;' id='newregline"+i+"' name='newregline"+i+"' onchange='updateValue(\""+dt[i].Patient_Id+"\",\"newregline"+i+"\",\"regimenline\");'> "+regimenlineoption+"</select></td><td><input onkeypress='return numbers(event);' value='"+dt[i].Last_Visit_Weight+"' onkeyup='isWeightCorrect(\""+i+"\");'  style='width:95%;' id='newweight"+i+"' name='newweight"+i+"' onchange='updateValue(\""+dt[i].Patient_Id+"\",\"newweight"+i+"\",\"weight\");' type='text' maxlength='2' minlength='2' class=''><label id='noofdigits"+i+"'></label></td></tr>";
...
//    } );
 ...
 function regimenlineoptionvalues(curoption)
    ...
    var opt=['First line','Second line','Third line'];
 ...
 function regimenoptionvalues(curoption)
        
    {   
        
    
    var sel="<option value=''>option</option>";
    var opt=["3TC+ABC+DTG+LOP+RTV+TDF","ABC+3TC","ABC+3TC+ATV/r","ABC+3TC+DTG","ABC+3TC+EFV","ABC+3TC+LPV/r","ABC+3TC+NVP","ABC+3TC+RAL","ABC+TDF+3TC+LPV/r","AZT+3TC+ATV/r","AZT+3TC+DTG","AZT+3TC+EFV","AZT+3TC+LPV/r","AZT+3TC+NVP","Other","TDF+3TC+ATV/r","TDF+3TC+DTG","TDF+3TC+EFV","TDF+3TC+LPV/r","TDF+3TC+NVP"];
  ...
  function updateValue(systemid,elem,elem_request_name)
  {
       ...
       if(elem.includes("newweight"))
       {   
            console.log("Weight Value Updated Successfully");
        if(isWeightOkay(elementvalue)) 
        {
         //save          
           blackbordercl(elem); 
          proceed=true;  
      ...
      // alert(da);
            ...
            if(proceed){
                $.ajax({
                    url: 'updateNewCalhivValues?sid='+sid+"&"+elem_request_name+"="+elementvalue,
                    type: 'post',
                    dataType: 'html',
                    success: function (data) {                      
