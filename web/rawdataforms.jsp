...
dbConn conn = new dbConn();                                 
                                 String getForms="select distinct(form) from pivottable where active=1";
                                 conn.rs=conn.st.executeQuery(getForms);
                                 while(conn.rs.next()){
                                 
                             
%>
                                       <option value="<%=conn.rs.getString("form")%>"><%=conn.rs.getString("form").toString().replace("vw_","").toUpperCase()%></option>                              
                                   <%}%>
                                 </select>
                              </div>
                             </div>
