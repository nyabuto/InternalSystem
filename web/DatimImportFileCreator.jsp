...
<head>
   <meta charset="utf-8" />
   <title>Generate Datim Import File</title>
   ...
   <div class="header navbar navbar-inverse navbar-fixed-top">
      ...
      <div class="navbar-inner">
         <div class="container-fluid">
            <!-- BEGIN LOGO -->
            <h2 style="text-align:center;font-size: 30px;color:white;padding-bottom:16px ;font-weight: bolder;">Datim File Import Creator </h2><br/>
...
<!--                             <div class="control-group">
                           ...
                           <input value="<%=ig.LastQuarterEndDate(ig.CurrentMonthEndDate())%>" required type="text" title=""  class="form-control input-lg" name="edate" id="edate" />
                             ...
                             <div class="control-group" >
                              ...
                              <div class="controls">
                                  <select multiple size="6"  required type="text" title="" onchange="selectoutput();loadDataToElement('getDatimFacilityCommunityIndicators','specificform','yes','','outiput');"  class="form-control input-lg" name="outiput" id="outiput" >
...
<!--                               <td class="col-xs-2">
  ...
  <label id="generaterpt" class="btn green" onclick="getReport();">Prepare Import File</label>
  <label id="generaterpt1" style="margin-left:200px;" class="btn blue" onclick="pullReport();">Download Import File</label>
  
  <label id="generaterpt2" style="margin-left:200px;" class="btn orange" onclick="deleteExisting();">Delete Existing Data</label>
   ...
   <!-- BEGIN FOOTER -->
    <div class="footer">
     ...
     <h4 class="portlet-title" style="text-align: center;color:black;"> &copy; Tujenge Jamii | USAID  Host Name :<b><i> </i></b> &nbsp;   Database Name :<i> </i></h4>
 ...
 $.ajax({
  ...
  function downloaddatimrpt()
  {
      ...
      var prd=$("#quarter").val();
                $('.loading').show();
                $('#generaterpt1').hide();
               var urel="DatimImportOutput";
                //?startdate=" + startdate + "&enddate=" + enddate + "&cbos=" + cbos
             
                var ur=urel+"?enddate=" + ed+"&startdate=" + sd+"&prd=" + prd;
                  console.log(ur);
                $.fileDownload(ur).done(function () { $('.loading').hide(); $('#generaterpt1').show(); $('#generaterpt1').html("<i class='glyphicon glyphicon-ok'></i> Report Generated"); }).fail(function () { alert('Report generation failed, kindly try again!'); $('.loading').hide(); $('#generaterpt1').show(); });
...
function loadDataToElement(act,element,issearchselect,firstoption,sourceoffilter)
{
  ...
  loadDataToElement('getDatimSiteandCommunitySections','outiput','yes','','');
  loadDataToElement('getDatimFacilityCommunityIndicators','specificform','yes','','outiput');
